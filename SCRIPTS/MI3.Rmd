...
```{R}
maine.auto <- auto.arima(e.ts.maine, approximation=FALSE, d=5)
summary(maine.auto)
```
...
# Forecast for next 120 days
forecasted_values <- forecast(fit, h = 120)

# Restore the original scale by adding the last value of the series
#forecasted_values$mean <- forecasted_values$mean + tail(maine.ts$mean, 1)
